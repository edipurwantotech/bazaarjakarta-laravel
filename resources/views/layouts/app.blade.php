...
<html lang="id">
  <head>
    ...
    @else
    <meta name="description" content="{{ \App\Models\Setting::getValue('seo_description', 'Bazaar Jakarta adalah platform bazaar online terbesar di Jakarta untuk jual beli produk lokal dan event komunitas. Temukan produk unik dan ikuti event menarik di kota Anda.') }}">
    @endif
  ...
  <body class="bg-gray-50 text-gray-800 font-poppins">
      ...
      <div class="container mx-auto flex justify-between items-center py-4 px-4 md:px-8">
        ...
        <nav class="hidden md:flex gap-6 font-semibold">
          @forelse($frontendMenus ?? [] as $menu)
                ...
                <div class="absolute top-full left-0 mt-2 w-48 bg-white rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
                  <div class="py-2">
                    @foreach($menu->children as $child)
                      <a href="{{ $child->url }}" class="block px-4 py-2 text-sm text-gray-800 hover:bg-orange-50 hover:text-orange-600 transition-colors {{ request()->is(ltrim($child->url, '/')) || request()->is(ltrim($child->url, '/*')) ? 'bg-orange-50 text-orange-600' : '' }}">
                        {{ $child->title }}
                      </a>
                    @endforeach
                  </div>
            ...
            @else
              <!-- Single Menu Item -->
              <a href="{{ $menu->url }}" class="hover:text-yellow-200 transition-colors duration-300 relative group {{ request()->is(ltrim($menu->url, '/')) || request()->is(ltrim($menu->url, '/*')) ? 'text-yellow-200' : '' }}">
                {{ $menu->title }}
                <span class="absolute bottom-0 left-0 w-0 h-0.5 bg-yellow-200 transition-all duration-300 group-hover:w-full {{ request()->is(ltrim($menu->url, '/')) || request()->is(ltrim($menu->url, '/*')) ? 'w-full' : '' }}"></span>
              </a>
            @endif
      ...
      <div class="hidden md:hidden bg-orange-700 px-4 py-3" id="mobile-menu">
        <nav class="flex flex-col gap-3 font-semibold">
          @forelse($frontendMenus ?? [] as $menu)
            @if($menu->children->count() > 0)
              ...
              <div class="relative">
                ...
                <div id="mobile-menu-{{ $menu->id }}" class="hidden pl-4 mt-2 space-y-2">
                  @foreach($menu->children as $child)
                    <a href="{{ $child->url }}" class="block py-1 text-sm hover:text-yellow-200 transition-colors">
                      {{ $child->title }}
                    </a>
                  @endforeach
                </div>
            ...
            @else
              <!-- Single Menu Item -->
              <a href="{{ $menu->url }}" class="hover:text-yellow-200 transition-colors duration-300">
                {{ $menu->title }}
              </a>
            @endif
          @empty
        ...
        <div class="grid md:grid-cols-4 gap-8">
          <div class="md:col-span-2">
            ...
            <div class="flex gap-3">
              ...
              <a href="{{ $socialMedia['facebook'] }}" target="_blank" class="bg-white/20 hover:bg-white/30 w-10 h-10 rounded-full flex items-center justify-center transition-colors">
                <i class="fab fa-facebook-f"></i>
              </a>
              @endif
              @if(isset($socialMedia['instagram']) && $socialMedia['instagram'] !== '#')
              <a href="{{ $socialMedia['instagram'] }}" target="_blank" class="bg-white/20 hover:bg-white/30 w-10 h-10 rounded-full flex items-center justify-center transition-colors">
                <i class="fab fa-instagram"></i>
              </a>
              @endif
              @if(isset($socialMedia['youtube']) && $socialMedia['youtube'] !== '#')
              <a href="{{ $socialMedia['youtube'] }}" target="_blank" class="bg-white/20 hover:bg-white/30 w-10 h-10 rounded-full flex items-center justify-center transition-colors">
                <i class="fab fa-youtube"></i>
              </a>
              @endif
              @if(isset($socialMedia['tiktok']) && $socialMedia['tiktok'] !== '#')
              <a href="{{ $socialMedia['tiktok'] }}" target="_blank" class="bg-white/20 hover:bg-white/30 w-10 h-10 rounded-full flex items-center justify-center transition-colors">
    ...
    <a href="https://wa.me/{{ preg_replace('/[^0-9+]/', '', $whatsappNumber) }}?text=Halo%20BazaarJakarta.ID,%20saya%20ingin%20bertanya%20tentang%20event%20anda"
       target="_blank"
