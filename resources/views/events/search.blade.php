...
@section('content')
<!-- BREADCRUMB SECTION -->
<section class="bg-cover bg-center bg-no-repeat text-white py-12 relative" style="background-image: url('{{ asset('images/bazaar-search-bg.jpg') }}');">
    <div class="absolute inset-0 bg-black bg-opacity-50"></div>
    <div class="container mx-auto px-4 md:px-8 relative z-10">
        <div class="text-center">
            <h1 class="text-3xl md:text-4xl font-bold mb-2">{{ $title }}</h1>
            <p class="text-orange-100">Temukan event yang sesuai dengan kebutuhan Anda</p>
        </div>
    </div>
</section>
...
<section class="py-16">
    <div class="container mx-auto px-4 md:px-8">
        @if($events->count() > 0)
            <div class="mb-8">
                <p class="text-gray-600">Ditemukan {{ $events->total() }} event untuk kata kunci "<span class="font-semibold">{{ $query }}</span>"</p>
            </div>
                        ...
                        <div class="p-6">
                            <div class="flex flex-wrap gap-2 mb-3">
                                @forelse($event->categories as $category)
                                    <a href="{{ route('events.category', $category->slug) }}" class="bg-orange-100 text-orange-600 text-xs px-2 py-1 rounded-full hover:bg-orange-200 transition-colors">{{ $category->name }}</a>
                                @empty
        ...
        @else
            <div class="text-center py-16">
                ...
                <h2 class="text-2xl font-bold text-gray-700 mb-4">Event Tidak Ditemukan</h2>
                <p class="text-gray-600 mb-8">Tidak ada event yang cocok dengan kata kunci "<span class="font-semibold">{{ $query }}</span>"</p>
