...
@section('content')
    ...
    @php
        ...
        if ($event->end_date) {
            $eventData["endDate"] = $event->end_date->format('Y-m-d') . ($event->time ? 'T' . $event->time->format('H:i') : '');
        }
        ...
        // Add images
        if ($event->thumbnail) {
            $eventData["image"][] = asset('storage/' . $event->thumbnail);
        }
        $eventData["image"][] = asset('images/logo-bazaar.png');
        ...
        <div class="max-w-4xl mx-auto">
          @if($event->thumbnail || $event->galleries->count() > 0)
            ...
            @if($event->thumbnail)
              <div class="mb-6">
                <img
                  src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 800 400'%3E%3Crect fill='%23f3f4f6' width='800' height='400'/%3E%3C/svg%3E"
                  data-src="{{ asset('storage/' . $event->thumbnail) }}"
                  alt="{{ $event->title }}"
                  class="w-full rounded-xl shadow-lg lazy"
                  loading="lazy"
                />
              </div>
            @endif
                ...
                @foreach($event->galleries as $gallery)
                  <div class="mb-4">
                    <img
                      ...
                      alt="{{ $gallery->caption ?? $event->title }}"
                      class="w-full rounded-xl shadow-lg lazy"
                      loading="lazy"
                    />
                    @if($gallery->caption)
          ...
          <div class="bg-white rounded-xl shadow overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
            <div class="h-48 relative overflow-hidden">
              @if($relatedEvent->thumbnail)
                <img
                  src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 400 300'%3E%3Crect fill='%23f3f4f6' width='400' height='300'/%3E%3C/svg%3E"
                  data-src="{{ asset('storage/' . $relatedEvent->thumbnail) }}"
